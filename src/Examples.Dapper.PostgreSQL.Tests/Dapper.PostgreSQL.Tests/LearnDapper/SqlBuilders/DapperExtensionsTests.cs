...
public class DapperExtensionsTests : IClassFixture<LearnDapperFixtures>
{
    public static bool IsDBAvailable => DatabaseEnvironment.IsAvailable;

    private readonly DbDataSource _dataSource;

    public DapperExtensionsTests(LearnDapperFixtures fixture, ITestOutputHelper output)
    {
        // sync
    ...
    public async Task WhenGetOperation()
    {
    ...
    [Fact(Skip = "DB is unavailable", SkipUnless = nameof(IsDBAvailable))]
    public async Task WhenGetListOperationWithPredicates()
    {
        using var connection = await _dataSource.OpenConnectionAsync(TestContext.Current.CancellationToken);
    ...
    [Fact(Skip = "DB is unavailable", SkipUnless = nameof(IsDBAvailable))]
    public async Task WhenSimpleInsertOperation()
    {
        using var connection = await _dataSource.OpenConnectionAsync(TestContext.Current.CancellationToken);
